...
function ResumeOptimizer() {
  ...
  async function handleFileUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    toast({
      title: "Resume loaded",
      description: `${file.name} — paste or edit below.`,
    });
  }
        ...
        {result && (
          <>
            ...
            <div className="rounded-xl border bg-white p-5 dark:bg-gray-900 dark:border-gray-800">
              ...
              <div className="mb-3">
                <p className="mb-2 text-xs text-red-600 dark:text-red-400 font-medium">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {result.missingKeywords.map((k) => (
                    <span
                      key={k}
                      className="rounded-full border border-red-200 bg-red-50 px-2.5 py-0.5 text-xs text-red-700 dark:border-red-800 dark:bg-red-950 dark:text-red-300"
                    >
                      {k}
                    </span>
                  ))}
              ...
              <div>
                <p className="mb-2 text-xs text-green-600 dark:text-green-400 font-medium">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {result.presentKeywords.map((k) => (
                    <span
                      key={k}
                      className="rounded-full border border-green-200 bg-green-50 px-2.5 py-0.5 text-xs text-green-700 dark:border-green-800 dark:bg-green-950 dark:text-green-300"
                    >
                      {k}
                    </span>
                  ))}
            ...
            <div className="rounded-xl border bg-white p-5 dark:bg-gray-900 dark:border-gray-800">
              <div className="flex items-center justify-between mb-3">
                ...
                <Button
                  ...
                  onClick={() => copyToClipboard(result.improvedSummary)}
                >
...
function CoverLetterGenerator() {
  ...
  const [tone, setTone] = useState<
    "confident" | "formal" | "friendly" | "concise" | "detailed"
  >("confident");
  const [loading, setLoading] = useState(false);
  ...
  async function generate() {
    if (!jobTitle || !company || !jobDesc) {
      toast({
        ...
        description: "Fill in job title, company, and description.",
        variant: "destructive",
      });
      return;
    }
      ...
      <div className="space-y-4 rounded-xl border bg-white p-5 dark:bg-gray-900 dark:border-gray-800">
        ...
        >
          {loading ? (
            ...
            <>
              <Zap className="h-4 w-4" /> Generate cover letter
            </>
          )}
        </Button>
      </div>
      ...
      <div className="rounded-xl border bg-white p-5 dark:bg-gray-900 dark:border-gray-800">
        ...
        <div className="mt-3 space-y-1.5">
          ...
          <Textarea
            value={jobDesc}
            onChange={(e) => setJobDesc(e.target.value)}
            placeholder="Paste JD for role-specific questions…"
            rows={4}
            className="resize-none text-xs"
          />
        </div>
        <Button
          onClick={generate}
          disabled={loading}
          className="mt-4 gap-2 bg-brand-600 hover:bg-brand-800 text-white"
        >
          {loading ? (
            <>
              <Loader2 className="h-4 w-4 animate-spin" /> Generating…
            ...
            <>
              <Zap className="h-4 w-4" /> Generate 8 questions
            </>
          )}
        </Button>
      </div>
      ...
      {questions.length > 0 && (
        <div className="space-y-3">
          {questions.map((q, i) => (
            <div
              key={i}
              className="rounded-xl border bg-white dark:bg-gray-900 dark:border-gray-800 overflow-hidden"
            >
              <button
